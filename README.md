...
This is a Flask HTTP server, which exposes a REST API to clients. It provides service for online restaurant ordering. A sample Android app using this service can be found at [my other GitHub repo here.](https://github.com/CristianoYL/RestaurantAndroidApp)
...
os.environ['APP_SECRET_KEY'] = <your_app_secret_key>
...
Stripe enforces security using Public Key Encryption. This is why you need to keep the ```STRIPE_SECRET_KEY``` in your ```config.py``` file secure. We communicate to Stripe using our Stripe Secret Key as well as a Public Key as follows:
...
A detailed API Reference can be found in the [API Reference.md](API%20Reference.md) file in the current repo.
